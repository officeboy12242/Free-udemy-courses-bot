...
# ─── My Channel (Telegram Streamer) ──────────────────────────────────────────

STREAM_SERVER_URL = os.getenv("STREAM_SERVER_URL", "http://localhost:10000")
STREAM_WORKER_URL = os.getenv("STREAM_WORKER_URL", "https://dl.yourname.workers.dev")

def mychannel_search(query: str, limit: int = 10) -> list[dict[str, str]]:
    """Search your own Telegram channel via the local stream_server API."""
    try:
        resp = requests.get(f"{STREAM_SERVER_URL}/search", params={"q": query}, timeout=15)
        if resp.status_code != 200:
            log.error("mychannel_search failed: HTTP %s", resp.status_code)
            return []
        ...
        for item in results:
            ...
            # Use caption as title if available, otherwise file name
            title = caption.split('\n')[0] if caption else file_name
            title = f"{title} ({size_str})"
            
            # The URL directly points to the Cloudflare Worker
            url = f"{STREAM_WORKER_URL}/watch/{msg_id}"
            
            movies.append({"title": title, "url": url, "poster": "", "source": "mychannel"})
            if len(movies) >= limit:
                break
                
        return movies
    except Exception as e:
        log.error("mychannel_search failed for '%s': %s", query, e)
        return []

def mychannel_movie_links(movie_url: str) -> dict[str, Any]:
    """For channel files, the URL is already the direct download/stream link."""
    return {
        "poster": "",
        "info": {},
        "links": [{"label": "▶️ Watch / Download File", "url": movie_url}],
        "episodes": [],
        "is_series": False,
    }

def format_mychannel_message(movie_title: str, data: dict, footer: bool = True) -> str:
    """Format a My Channel result as HTML for Telegram."""
    links = data.get("links", [])
    lines: list[str] = []
    disp = movie_title or data.get("title", "")
    if disp:
    ...
    lines.append("\n" + "━" * 32)
    lines.append("📥 <b>Direct Stream/Download Link</b>\n")
    for lk in links:
        label = lk.get("label", "Download")
        url_ = lk.get("url", "")
        lines.append(f"📦 <b>{label}</b>\n   🔗 <a href=\"{url_}\">Click Here</a>")

    if footer:
        lines.append("\n" + "━" * 32)
        lines.append('⚡ Powered by <a href="https://t.me/CoursesDrivee">@CoursesDrivee</a>')
